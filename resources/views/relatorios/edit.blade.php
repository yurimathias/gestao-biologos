                ...
                <div class="form-group">
                    ...
                    <select name="biologo_id" id="biologo_id" required>
                        ...
                        @foreach ($biologos as $biologo)
                            <option value="{{ $biologo->id }}" @selected($relatorio->biologo_id == $biologo->id)>{{ $biologo->nome }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select name="area_id" id="area_id" required>
                        ...
                        @foreach ($areas as $area)
                            <option value="{{ $area->id }}" @selected($relatorio->area_id == $area->id)>{{ $area->nome }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <x-input-label for="observacoes" :value="__('Observações')" />
                    <textarea name="observacoes" id="observacoes" cols="30" rows="5" required>{{ $relatorio->observacoes }}</textarea>
                </div>
                <div class="form-group">
                    ...
                    <select class="select2-multiple" name="animal_id[]" id="animal_id" multiple>
                        ...
                        @foreach ($animais as $animal)
                            <option value="{{ $animal->id }}" @selected($relatorio->animais->contains('id', $animal->id))>{{ $animal->nome_cientifico . ' | ' . $animal->nome_comum }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select class="select2-multiple" name="planta_id[]" id="planta_id" multiple>
                        ...
                        @foreach ($plantas as $planta)
                            <option value="{{ $planta->id }}" @selected($relatorio->plantas->contains('id', $planta->id))>{{ $planta->nome_cientifico . ' | ' . $planta->nome_comum }}</option>
                        @endforeach
                    </select>
                </div>
